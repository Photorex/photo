    ...
    async signIn({ user, account, profile }) {
      if (account && account.provider === "google") {
        try {
          ...
          const photorendRefcode = allCookies.get("photorend_refcode");
          const fingerprintIdCookie = allCookies.get("fingerprintId");

          // Log the cookie value for debugging
          // console.log("photorend_refcode Cookie:", photorendRefcode?.value);

          if (photorendRefcode?.value) {
              referralCode = photorendRefcode.value;
